...
const TargetCursor = ({
  ...
  useEffect(() => {
    const checkDevice = () => {
      ...
      const isTablet = window.innerWidth <= 1024; // Increased threshold to include iPad Pro
  ...
  }, [targetSelector, spinDuration, moveCursor, constants, hideDefaultCursor, isDesktop]);

  useEffect(() => {
    if (!cursorRef.current || !spinTl.current || !isDesktop) return;
